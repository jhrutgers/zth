 ...
 *
 * Use one of the guards in \c zth::guards, like \c zth::guards::always(), or
 * define your own. A guard is a function that gets the current Fsm reference
...
#  endif
	 ...
	 * \endverbatim
	 *
	 * Normally, the transitions have guards that determine if the Fsm is
	 * allowed to continue.  In lockstep mode, one add special states where
	 * the Fsm can wait for an explicit input.  For example, if such
	 * explicit input is required before continuing back to A:
	 *
	 * \verbatim
	 * +--> A ---+
	 * |         |
	 ...
	 * \endverbatim
	 *
	 * In this Fsm, once B is reached, the Fsm may go to the stalled state
	 * C, wait for an explicit input, go back to B and proceed to A.  To
	 * implement this, add a guard that checks #guardLock() at entry of B.
	 * If it returns \c true, proceed to C. In C, add a guard that checks
	 * #guardStep().  If it returns \c true, proceed back to B. Any
	 * #guardLock() guards are ignored until state B is left. If the
	 * expected explicit input was received, call #step().
	 */
...
protected:
	/*!
	 * \brief This function is called when the FSM has to execute the code
	 *	belonging by the current state.
	 *
	 * It is called upon entry and exit of a state, and arbitrary times
	 * during a state.
	 */
